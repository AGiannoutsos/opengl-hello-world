...
int main(){
    ...
    Model planetModel(FileSystem::getPath(PLANET_MODEL_PATH));

    Shader planetShader("shaders/planet_shader.vs", "shaders/planet_shader.fs");
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        // *************************************************
        planetShader.use();
        glm::mat4 planetPos = glm::mat4(1.0f);
        ...
        planetShader.setMat4("model", planetPos);
        ...
        // ***********************************************
        containerShader.use();
        // light has the same position with the planet
        containerShader.setVec3("light.position", glm::vec3(planetPos * glm::vec4(POINT_ZERO, 1.0)));
        ...
        for (int container = 0; container < NUM_OF_CONTAINERS; container++){
            // calculate the orbit around the planet
        ...
        // light position = planet position
        glm::mat4 lightPos = planetPos;
